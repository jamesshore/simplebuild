	...
	exports.check = function(arg, type, options) {
		...
		for (var i = 0; i < type.length; i++) {
			if (oneTypeMatches(arg, argType, type[i])) {
				if (isStructComparison(argType, type[i])) return checkStruct(arg, type[i]);
				else return null;
			}
		}
		...
		function checkStruct(arg, type) {
			...
			for (var i = 0; i < keys.length; i++) {
				var checkResult = exports.check(arg[keys[i]], type[keys[i]]);
				if (checkResult !== null) return describeError(argType, type);
			}
