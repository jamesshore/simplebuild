...
exports.normalizeOptions = function(defaultsAndNormalizedObject, userOptions, types) {
	var name = { name: "options" };

	var notAnObject = type.check(userOptions, Object, name);
	if (notAnObject) return notAnObject;

	objectAssign(defaultsAndNormalizedObject, userOptions);
	return type.check(defaultsAndNormalizedObject, types, name);
};
